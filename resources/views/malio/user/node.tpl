                    ...
                    <div class="row">
                        ...
                        <div class="col-12 col-sm-12 col-lg-6">
                            ...
                            <div class="card" {if $user->class>0} data-toggle="modal" data-target="#node-modal-{$node['id']}"{/if}>
                                {else}
                                <div class="card"
                                     {if $user->class >0}onclick="urlChange('{$node['id']}',0,{if $relay_rule != null}{$relay_rule->id}{else}0{/if})"{/if}>
                                    {/if}
                                    <div class="card-body">
                                        <ul class="list-unstyled user-details list-unstyled-border list-unstyled-noborder">
                                            <li class="media">
                                                ...
                                                <img alt="image" class="mr-3 rounded-circle" width="50"
                                                     src="{if {$malio_config['malio_js_url']} == ''}/theme/malio{else}{$malio_config['malio_js_url']}npm/malio{/if}/assets/modules/flag-icon-css/flags/1x1/{if $malio_config['flag_mode']=='node-name'}{if $flags[$region] != ''}{$flags[$region]}{else}un{/if}{else}{$node['status']}{/if}.svg">
                                    ...
                                    {if $config['show_stream_media'] == true && !empty($node['unlock'])}
                                        ...
                                        <div>
                                            <table id="node_unlocak" style="width: 100%;">
                                                <thead>
                                                <tr align="center">
                                                    <th style="width: 20%;"><span>YouTube</span>
                                                    </th>
                                                    <th style="width: 20%;"><span>Netflix</span>
                                                    </th>
                                                    <th style="width: 20%;">
                                                        <span>DisneyPlus</span></th>
                                                    <th style="width: 20%;"><span>Blibli</span>
                                                    </th>
                                                    <!-- <th data-field="台湾B站" class="datatable-cell"><span>台湾B站</span></th>-->
                                                    {*                                                    <th data-field="MyTVSuper" class="datatable-cell">*}
                                                    {*                                                        <span>MyTVSuper</span></th>*}
                                                ...
                                                <tr align="center">
                                                    ...
                                                    <td>
                                                        {if ({$node['unlock']['unlock_item']['BilibiliHKMCTW']}=='Yes') || ({$node['unlock']['unlock_item']['BilibiliTW']}=='Yes')}
                                                            <span>Yes</span>
                                                        {/if}
                                                        {if {$node['unlock']['unlock_item']['BilibiliTW']}=='No' && {$node['unlock']['unlock_item']['BilibiliHKMCTW']}=='No'}
                                                            <span>No</span>
                                                        {/if}
                                                    </td>
                                                    ...
                                                    {*                                                        </td>*}
                                                    {*                                                    <td data-field="MyTVSuper" style="width: 13%;"*}
                                                    {*                                                        aria-label="{$node['unlock']['unlock_item']['MyTVSuper']}"*}
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-2">{$i18n->get('transport')}:
                                <code>{if $v2server['net']=="tls"}tcp{else}{$v2server['net']}{/if}</code></div>
                            {if $v2server['net']=="ws"}
                                <div class="mb-2">{$i18n->get('path')}: <code>{$v2server['path']}</code></div>
                            {/if}
                            {if $v2server['net']=="kcp"}
                                <div class="mb-2">{$i18n->get('camouflage-type')}: <code>{$v2server['type']}</code>
                                </div>
                            {/if}
                            ...
                            {/if}
                            <div class="mb-2">{$i18n->get('vmess-url')}:
                                <code>{URL::getV2Url($user, $node['raw_node'])}</code></div>
                        </div>
                    </div>
                </div>
            </div>
